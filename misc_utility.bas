...
'******************************************************************************
Sub testpp()
    Dim m As Variant, pp As Variant, g As Variant
    m = makeM(300, 6, uniform_int_dist(300 * 6, 1, 15))
    m = subM(m, sortIndex(m))
    pp = partition_points(selectCol(m, 0))
    g = group_by_partition_points(m, pp, "%t%c%s%a%min%max")
    With Sheets("Sheet5")
        .[A2:M302].ClearContents
        m2sheet m, .[a2]
        m2sheet g, .[H2]
    End With
    m = makeM(6, 4, uniform_int_dist(4 * 6, 1, 15))
    Call columnWise_change(m, splitStr2Funs("%%s%%", p_str2ConvertFun, "%"))
    'printM mapF(p_typename, m)
End Sub
'******************************************************************************

Public Function trimSplit(ByRef expr As Variant, ByRef delimiter As Variant) As Variant
    trimSplit = Split(expr, delimiter)
    trimSplit = filterR(trimSplit, mapF(p_len, trimSplit))
End Function
    Public Function p_trimSplit(Optional ByRef firstParam As Variant, Optional ByRef secondParam As Variant) As Variant
        p_trimSplit = make_funPointer(AddressOf trimSplit, firstParam, secondParam)
    End Function

Public Function stat_median(ByRef m As Variant, Optional ByRef pred As Variant) As Variant
    Dim s As Variant
    If IsMissing(pred) Then
        s = sortIndex(m)
    Else
        s = sortIndex_pred(m, pred)
    End If
    Dim size_ As Long:      size_ = sizeof(m)
    If size_ Mod 2 = 1 Then
        stat_median = m(getNth_b(s, (size_ - 1) / 2))
    Else
        stat_median = (m(getNth_b(s, -1 + size_ / 2)) + m(getNth_b(s, size_ / 2))) / 2#
    End If
End Function
    Public Function p_stat_median(Optional ByRef firstParam As Variant, Optional ByRef secondParam As Variant) As Variant
        p_stat_median = make_funPointer(AddressOf stat_median, firstParam, secondParam)
    End Function

Public Function stat_mode(ByRef m As Variant, Optional ByRef pred As Variant) As Variant
    Dim s As Variant
    Dim pp As Variant
    If IsMissing(pred) Then
        s = sortIndex(m)
    ...
    Else
        s = sortIndex_pred(m, pred)
        permutate m, s
        pp = partition_points_pred(m, pred)
    End If
    permutate_back m, s
    Dim intervals As Variant
    intervals = adjacent_op(p_minus, pp)
    stat_mode = filterR(pp, mapF(p_equal(foldl1(p_min, intervals))))
End Function
    Public Function p_stat_mode(Optional ByRef firstParam As Variant, Optional ByRef secondParam As Variant) As Variant
        p_stat_mode = make_funPointer(AddressOf stat_mode, firstParam, secondParam)
    End Function
