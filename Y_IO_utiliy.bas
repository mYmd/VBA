...
Public Function urlDecode(ByVal s As String) As String
    If s Like "*%??*" Then
        ...
        Set ado = CreateObject("ADODB.Stream")
        Dim mid3 As String
        Do While begin <= Len(s)
            mid3 = Mid(s, begin, 3)
            If mid3 Like "%??*" Then
                If mid3 < "%7F" Then
                    urlDecode = urlDecode & urlDecode_imple(mid3, ado)
                    begin = begin + 3
                ElseIf Mid(s, begin + 3, 6) Like "%??%??*" Then
                    urlDecode = urlDecode & urlDecode_imple(Mid(s, begin, 9), ado)
                    begin = begin + 9
                ElseIf Mid(s, begin + 3, 3) Like "%??*" Then
                    urlDecode = urlDecode & urlDecode_imple(Mid(s, begin, 6), ado)
                    begin = begin + 6
                Else
                    begin = begin + 3   '異常
                End If
...
End Function
    ...
    Private Function isKanaKanji(ByVal s As String) As Boolean
        isKanaKanji = True
        If 0 < Len(s) Then
            Dim s1 As Long: s1 = Asc(Left(s, 1))
            Select Case True
                Case s1 = 45 Or s1 = 46:        isKanaKanji = False '   "-", "."
                Case 48 <= s1 And s1 <= 57:     isKanaKanji = False '   "0"-"9"
                Case 65 <= s1 And s1 <= 90:     isKanaKanji = False '   "A"-"Z"
                Case 95 = s1:                   isKanaKanji = False '   "_"
                Case 97 <= s1 And s1 <= 122:    isKanaKanji = False '   "a"-"z"
                Case 126 = s1:                  isKanaKanji = False '   "~"
            End Select
        End If
    End Function
    ...
    Private Function urlEncode_1(ByRef s As Variant, ByRef adodbStream As Variant) As Variant
        ...
        If isKanaKanji(s) Then
            With adodbStream
                ...
                .Close
                urlEncode_1 = ""
                For i = LBound(chars) To UBound(chars) Step 1
                    urlEncode_1 = urlEncode_1 & "%" & Hex(chars(i))
                Next i
            End With
        Else
            urlEncode_1 = s
    ...
    Private Function urlDecode_imple(ByVal code As String, ByVal adodbStream As Object) As String
        Dim chars() As Byte, i As Long
        ReDim chars(0 To -1 + Len(code) / 3)
        For i = LBound(chars) To UBound(chars) Step 1
            chars(i) = CLng("&H" & Mid(code, i * 3 + 2, 2))
        Next i
        With adodbStream
            .Open
            .Type = 1       'ADODB.Stream.adTypeBinary
